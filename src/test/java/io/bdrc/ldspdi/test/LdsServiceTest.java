...
public class LdsServiceTest extends JerseyTest {
	...
	public static void init() {
		...
		//setPrefixes(model);
		loadData();		
		srvds.addNamedModel("test", model);
	...
	*/
	...
	private Model[] prepareAssertModel(String res) throws IOException{
		...
		Model m=addModelFromFileName(TestUtils.TESTDIR+res+".ttl", sttl);
		Map<String,String> prefixMap=m.getNsPrefixMap();
		String prefix=TestUtils.convertToString(prefixMap);
	...
	static void loadData(){
		//loads some ttl into the main model
		ArrayList<String> list=TestUtils.getResourcesList();
		for(String res : list){
			addModelFromFileName(TestUtils.TESTDIR+res+".ttl", sttl);
		}		
	}
	
	static Model addModelFromFileName(String fname, Lang lang) {
