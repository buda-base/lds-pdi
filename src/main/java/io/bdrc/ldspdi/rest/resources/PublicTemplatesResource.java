...
public class PublicTemplatesResource {
    ...
    public Response getRawQueryTemplateResults(@Context UriInfo info, @HeaderParam("fusekiUrl") final String fusekiUrl, @PathParam("file") String file) throws RestException {
        log.info("Call to getQueryTemplateResults() {}, params: {}", file, info.getQueryParameters());
        // Settings
        HashMap<String, String> hm = Helpers.convertMulti(info.getQueryParameters());
        hm.put(QueryConstants.REQ_URI, info.getRequestUri().toString().replace(info.getBaseUri().toString(), "/"));
        hm.put(QueryConstants.REQ_METHOD, "GET");
        // process
        final LdsQuery qfp = LdsQueryService.get(file + ".arq", "private");
        final String query = qfp.getPrefixedQuery();
        ResultSetWrapper res = QueryProcessor.getResults(query, fusekiUrl, hm.get(QueryConstants.RESULT_HASH), Integer.toString(Integer.MAX_VALUE));
        String fmt = hm.get(QueryConstants.FORMAT);
        if ("json".equals(fmt)) {
            return Response.ok(ResponseOutputStream.getJsonResponseStream(new Results(res, hm)), MediaType.APPLICATION_JSON_TYPE).header("Content-Disposition", "attachment; filename=\"" + file + ".json\"").build();
        }
    ...
    public Response testTemplateResults(@Context UriInfo info, @HeaderParam("fusekiUrl") final String fuseki, @PathParam("file") String file) throws RestException {
