...
<c:if test = "${model.isPresent()}">
    ...
    <c:if test = "${model.getAdminAnnotProps().size()>0}">
        ...
        <c:forEach items="${model.getAdminAnnotProps()}" var="prop">
            <div style="white-space: pre-wrap;"><b>${Prefixes.getPrefix(prop.getNameSpace())}:${prop.getLocalName()}</b><span>&nbsp;:&nbsp;${model.getPropertyValue(prop).asLiteral().getString()}</span><span class="lang">${model.getPropertyValue(prop).asLiteral().getLanguage()}</span></div><hr/>
        </c:forEach>
    </c:if>
